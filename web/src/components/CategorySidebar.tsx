    ...
    <nav className="flex w-56 shrink-0 flex-col gap-1 border-r border-vip-border p-3">
      ...
      {categories.map((category) => {
        ...
        return (
          ...
          >
            {isActive && <span className="absolute left-0 top-1/2 h-4 w-[3px] -translate-y-1/2 rounded-full bg-vip-accent-soft" />}
            <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-vip-accent-soft' : 'text-vip-muted group-hover:text-vip-text'}`} />
            <span className="flex-1 truncate font-medium">{category.label}</span>
            <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${isActive ? 'bg-vip-accent/20 text-vip-accent-soft' : 'bg-vip-panel-2 text-vip-muted'}`}>
              {count}
            </span>
          </button>
        );

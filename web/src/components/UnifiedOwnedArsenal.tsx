...
export function UnifiedOwnedArsenal() {
  ...
  const compatibleSkins = useMemo(() => selected ? skins.filter((skin) => compatible(skin, selected.weapon)) : [], [skins, selected]);
  ...
  return (
    <motion.section initial={{ opacity: 0, scale: .975, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="flex h-[min(760px,93vh)] w-[min(1320px,96vw)] overflow-hidden rounded-[22px] border border-white/[0.07] bg-[#0b0e13]/96 shadow-[0_22px_70px_rgba(0,0,0,.45)]">
      <aside className="flex w-[270px] shrink-0 flex-col border-r border-white/[0.06] bg-[#090c11]">
        <div className="p-5">
          <div className="flex items-center gap-3"><Crown className="h-5 w-5 text-[#ff8a1f]" /><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#ff8a1f]">{t(translations,'ui_owned_my_arsenal')}</p><p className="text-[10px] text-vip-muted">{ownerName}</p></div></div>
        </div>

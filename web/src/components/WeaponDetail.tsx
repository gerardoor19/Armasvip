...
export function WeaponDetail() {
  ...
  return (
    <motion.aside key={selectedWeapon.name} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.18 }} className="flex w-96 shrink-0 flex-col border-l border-vip-border p-5">
      <div className="relative flex h-32 items-center justify-center overflow-hidden rounded-xl bg-vip-panel-2/60"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(255,122,0,0.1),transparent_65%)]" />{imgError ? <Crosshair className="h-12 w-12 text-vip-muted" /> : <img src={`${imageBase}${selectedWeapon.name}.png`} alt={selectedWeapon.label} className="relative h-32 max-w-full object-contain drop-shadow-[0_6px_16px_rgba(0,0,0,0.6)]" onError={() => setImgError(true)} draggable={false} />}</div>
      <h2 className="mt-3 text-center text-base tracking-wide text-vip-text">{selectedWeapon.label}</h2><p className="text-center text-[11px] uppercase tracking-wide text-vip-cyan/80">{categoryLabel}</p>

      <div className="mt-4 flex-1 space-y-4 overflow-y-auto pr-1">
        {groupedComponents.length === 0 ? <p className="text-center text-xs text-vip-muted">{t(translations, 'ui_no_components')}</p> : groupedComponents.map(({ type, items }) => <div key={type}><h3 className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-vip-muted">{t(translations, componentTypeTranslationKeys[type] ?? 'ui_component_other', type)}</h3><div className="flex flex-wrap gap-1.5">{items.map((name) => { const isActive = selectedComponents.includes(name); return <button key={name} onClick={() => toggleComponent(name)} className={`flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs transition-colors duration-150 ${isActive ? 'border-vip-accent/60 bg-vip-accent/15 text-vip-accent-soft' : 'border-vip-border bg-vip-panel-2 text-vip-muted hover:text-vip-text'}`}>{isActive && <Check className="h-3 w-3" strokeWidth={3} />}{componentsMeta[name]?.label ?? name}</button>; })}</div></div>)}
        ...
        <div className="border-t border-white/[0.06] pt-4">
          <div className="flex items-center justify-between"><h3 className="text-[11px] font-semibold uppercase tracking-wide text-vip-muted">{t(translations, 'ui_skin_collection')}</h3><span className="text-[9px] text-vip-muted">{selectedWeapon.skinSupported === false ? '0/0' : `${selectedSkins.length}/${compatibleSkins.length}`}</span></div>
